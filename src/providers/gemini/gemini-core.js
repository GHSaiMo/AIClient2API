        ...
        this.authClient.on('tokens', async (tokens) => {
            try {
                if (this.oauthCredsBase64) {
                    logger.info('[Gemini Auth] Tokens refreshed in memory for Base64 credential source.');
                    return;
                }
                ...
                logger.info(`[Gemini Auth] Captured 'tokens' event and updated credentials in ${credPath}`);
                const poolManager = getProviderPoolManager();
                if (poolManager && this.uuid) {
                    poolManager.resetProviderRefreshStatus(this.config.MODEL_PROVIDER || MODEL_PROVIDER.GEMINI_CLI, this.uuid);
                }
            } catch (err) {
                logger.warn(`[Gemini Auth] Failed to save updated tokens on 'tokens' event: ${err.message}`);
            }
        });
    }
